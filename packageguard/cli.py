...
def print_report(report, llm_name=None):
    ...
    if package_trust.get("new_package"):
        print(_style("!!! RECENTLY PUBLISHED OR UPDATED PACKAGE !!!", *_severity_style("CRITICAL")))
        print("-" * 60)
        print("This package was published or modified within the last 24 hours.")
        print("This does not prove it is malicious, but it may increase trust risk because the community has had less time to review, report, or flag the release.")
    ...
    if metadata.get("downloads_last_week") is not None:
        ...
        print(f"{metadata.get('status')}")
        print(f"{metadata.get('downloads_last_week')}")
        print(f"{metadata.get('age_days')}")
        print(f"{metadata.get('days_since_modified')}")
        print("")
        print("")
    ...
    new_package = package_trust.get("new_package", False)
